...
const Sidebar: React.FC = () => {
  ...
  const handleDelete = async (itemId: string) => {
    ...
    try {
      ...
      toast.success(t('meetingDeleted'), {
        description: t('associatedDataRemoved')
      });

      // If deleting the active meeting, navigate to home
      if (currentMeeting?.id === itemId) {
        setCurrentMeeting({ id: 'intro-call', title: t('newMeeting') });
        router.push('/');
      }
    } catch (error) {
  ...
  const handleEditConfirm = async () => {
    ...
    if (!newTitle) {
      toast.error(t('meetingTitleEmpty'));
      return;
    }
    ...
    try {
      ...
      toast.error(t('updateMeetingTitleFailed'), {
        description: error instanceof Error ? error.message : String(error)
      });
    }
  };
            ...
            {!isCollapsed && (
              <div className="p-3">
                ...
                <div className="relative mb-1">
                  <InputGroup >
                    <InputGroupInput placeholder={t('searchMeetingContent')} value={searchQuery}
