...
export const RecordingControls: React.FC<RecordingControlsProps> = ({
  ...
  useEffect(() => {
    const checkTauri = async () => {
      try {
        ...
        alert(t('recordingInitializationFailed'));
      }
    };
    checkTauri();
  }, [t]);
  ...
  const handleStartRecording = useCallback(async () => {
    ...
    try {
      ...
      if (normalizedError.includes('microphone') || normalizedError.includes('mic') || normalizedError.includes('input')) {
        setDeviceError({
          title: t('microphoneUnavailable'),
          message: `${t('microphoneUnavailableDescription')}\n${errorMsg}`
        });
      } else if (normalizedError.includes('system audio') || normalizedError.includes('speaker') || normalizedError.includes('output')) {
        setDeviceError({
          title: t('systemAudioUnavailable'),
          message: `${t('systemAudioUnavailableDescription')}\n${errorMsg}`
        });
      } else if (normalizedError.includes('permission')) {
        setDeviceError({
          title: t('permissionRequired'),
          message: `${t('permissionRequiredDescription')}\n${errorMsg}`
        });
      } else {
        setDeviceError({
          title: t('recordingFailed'),
          message: `${t('recordingFailedDescription')}\n${errorMsg}`
        });
      }
    }
        ...
        <div className="flex items-center space-x-2 bg-white rounded-full shadow-lg px-4 py-2">
          {isProcessing && !isParentProcessing ? (
            <div className="flex items-center space-x-2">
              <Loader2 className="h-5 w-5 animate-spin text-gray-900" aria-hidden="true" />
              <span className="text-sm text-gray-600">{t('processingRecording')}</span>
            </div>
          ) : (
            <>
              {showPlayback ? (
                  ...
                  {!isRecording ? (
                    ...
                    <>
                      <Tooltip>
                        ...
                        <TooltipContent>
                          <p>{isPaused ? t('resumeRecording') : t('pauseRecording')}</p>
                        </TooltipContent>
                      </Tooltip>
