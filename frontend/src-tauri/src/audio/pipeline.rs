...
impl AudioPipeline {
                ...
                Ok(Some(chunk)) => {
                    ...
                    while self.ring_buffer.can_mix() {
                        if let Some((mic_window, sys_window)) = self.ring_buffer.extract_window() {
                            ...
                            // STEP 3: Run VAD only when live transcription is enabled.
                            if let (Some(transcription_sender), Some(vad_processor)) = (
                                self.transcription_sender.clone(),
                                self.vad_processor.as_mut(),
                            ) {
                                match vad_processor.process_audio(&mixed_with_gain) {
                                    Ok(speech_segments) => {
    ...
    fn flush_remaining_audio(&mut self) -> Result<()> {
        ...
        let Some(transcription_sender) = self.transcription_sender.clone() else {
            return Ok(());
        };
        let Some(vad_processor) = self.vad_processor.as_mut() else {
            return Ok(());
        };

        // Flush any remaining audio from VAD processor and send segments to transcription
        match vad_processor.flush() {
            Ok(final_segments) => {
