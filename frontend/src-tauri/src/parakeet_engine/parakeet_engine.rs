...
impl ParakeetEngine {
    ...
    pub async fn delete_model(&self, model_name: &str) -> Result<String> {
        ...
        // An externally referenced model is a symbolic link. Removing it must
        // never recurse into or delete the original directory.
        if std::fs::symlink_metadata(&model_info.path)
            .map(|metadata| metadata.file_type().is_symlink())
            .unwrap_or(false)
        {
            #[cfg(unix)]
            fs::remove_file(&model_info.path).await.map_err(|e| {
                anyhow!(
                    "Failed to remove model reference '{}': {}",
                    model_info.path.display(),
                    e
                )
            })?;
            #[cfg(windows)]
            fs::remove_dir(&model_info.path).await.map_err(|e| {
                anyhow!(
                    "Failed to remove model reference '{}': {}",
                    model_info.path.display(),
                    e
            ...
            return Ok(format!(
                "Successfully removed Parakeet model reference '{}'",
                model_name
            ));
        }
